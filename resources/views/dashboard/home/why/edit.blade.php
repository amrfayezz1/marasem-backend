...
<div class="container">
    ...
    <form action="{{ route('why.update', $why->id) }}" method="POST">
        ...
        <div class="form-group mt-3">
            ...
            <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $why->title) }}"
                required>
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <textarea name="description" id="description" class="form-control" rows="4"
                required>{{ old('description', $why->description) }}</textarea>
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <select name="icon" id="icon" class="form-select">
                @foreach ($icons as $icon)
                    <option value="{{ $icon }}" data-icon="{{ $icon }}" {{ $why->icon === $icon ? 'selected' : '' }}>
                        {{ $icon }}
                    </option>
                @endforeach
            </select>
        ...
        <a href="{{ route('why.index') }}" class="btn btn-secondary mt-4">Cancel</a>
    </form>
</div>
